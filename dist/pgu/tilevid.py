...
class Tilevid(Vid):
    ...
    def paint(self, s):
        ...
        if blayer != None:
            for y in range(old_div(oy, th), my):
                if y >= 0 and y < h:
                    trow = tlayer[y]
                    brow = blayer[y]
                    arow = alayer[y]
                    ...
                    mx = old_div((ox + sw), tw)
                    # if (ox+sh)%tw: mx += 1
                    for x in range(old_div(ox, tw), mx + 1):
                        if x >= 0 and x < w:
    ...
    def update(self, s):
        ...
        for s in ss:
            ...
            if s.updated:
                r = s._irect
                y = max(0, old_div(r.y, th))
                yy = min(h, old_div(r.bottom, th) + 1)
                while y < yy:
                    x = max(0, old_div(r.x, tw))
                    xx = min(w, old_div(r.right, tw) + 1)
                    while x < xx:
                        if alayer[y][x] == 0:
    ...
    def view_to_tile(self, pos):
        ...
        tiles = self.tiles
        tw, th = tiles[0].image.get_width(), tiles[0].image.get_height()
        return old_div(x, tw), old_div(y, th)
