...
class Hexvid(Vid):
    ...
    def paint(self, screen):
        ...
        tile_w2, tile_h2 = old_div(tile_w, 2), old_div(tile_h, 2)
        ...
        tile_wi = tile_w + old_div(tile_w, 2)
        tile_wi2 = old_div(tile_wi, 2)

        # dx += tile_w/2

        for i2 in range(-bot, old_div(self.view.h, tile_h2) + bot * 3):  # NOTE: 3 seems a bit much, but it works.
            ...
            x -= old_div(tile_w, 2)
            for i1 in range(-1, old_div(self.view.w, tile_wi) + 1):
    ...
    def view_to_tile(self, pos):
        ...
        x, y = int(old_div(x * 4, (self.tile_w * 3))), old_div(y * 2, self.tile_h)
        nx = old_div((x + y), 2)
        ny = old_div((y - x), 2)
        return nx, ny
    ...
    def tile_to_view(self, pos):
        ...
        nx = x - y
        ny = x + y
        nx, ny = int(old_div(nx * (self.tile_w * 3), 4)), old_div(ny * self.tile_h, 2)

        # nx = nx - (self.tile_w*1/2)
        return nx, ny

    def screen_to_tile(self, pos):  # NOTE HACK : not sure if the 3/8 is right or not, but it is pretty close...
        pos = pos[0] + self.view.x + old_div(self.tile_w * 3, 8), pos[1] + self.view.y
