...
class Isovid(Vid):
    ...
    def paint(self, screen):
        ...
        base_h2 = old_div(base_h, 2)
        base_w2 = old_div(base_w, 2)

        bot = old_div(tile_h, base_h2)
        todo_max = old_div(sh, base_h2) + bot
        ...
        for i2 in range(-bot, old_div(self.view.h, base_h2) + bot):
            tx, ty = ox + old_div(i2, 2) + i2 % 2, oy + old_div(i2, 2)
            x, y = (i2 % 2) * base_w2 + dx, i2 * base_h2 + dy

            # to adjust for the -1 in i1
            x, tx, ty = x - base_w, tx - 1, ty + 1
            for i1 in range(-1, old_div(self.view.w, base_w) + 2):  # NOTE: not sure why +2
                ...
                ty -= 1
                x += base_w
            for img, irect in todo[old_div(y, base_h2)]:
    ...
    def iso_to_view(self, pos):
        ...
        nx, ny = old_div((x - y), 2), old_div((0 + x + y), 2)

        return (old_div(nx * self.base_w, self.iso_w)), (old_div(ny * self.base_h, self.iso_h))
    ...
    def view_to_iso(self, pos):
        ...
        x, y = old_div(x * self.iso_w, self.base_w), old_div(y * self.iso_h, self.base_h)
    ...
    def screen_to_tile(self, pos):
        ...
        return old_div(x, self.iso_w), old_div(y, self.iso_h)
    ...
    def tga_load_tiles(self, fname, size, tdata={}):
        ...
        self.base_w, self.base_h = self.tile_w, old_div(self.tile_w, 2)
    ...
    def sprite_calc_irect(self, s):
        ...
        x, y = self.iso_to_view((s.rect.centerx, s.rect.centery))
        tx, ty = old_div(s.rect.centerx, self.iso_w), old_div(s.rect.centery, self.iso_h)
        z = 0
        if ty >= 0 and ty < h and tx >= 0 and tx < w:
