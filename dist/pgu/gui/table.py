    ...
    def resize(self, width=None, height=None):
                ...
                if self._rows[row][cell] and self._rows[row][cell] is not True:
                    if self._rows[row][cell]["colspan"] > 1:
                        ...
                        if totalwidth < self._rows[row][cell]["widget"].rect.w:
                            for acol in columns:
                                columnsizes[acol] += _table_div(self._rows[row][cell]["widget"].rect.w - totalwidth,
                                                                self._rows[row][cell]["colspan"], acol)
                    if self._rows[row][cell]["rowspan"] > 1:
                        ...
                        if totalheight < self._rows[row][cell]["widget"].rect.h:
                            for arow in rows:
                                rowsizes[arow] += _table_div(self._rows[row][cell]["widget"].rect.h - totalheight,
                                                             self._rows[row][cell]["rowspan"], arow)
        ...
        if w > 0 and w < self.style.width and len(columnsizes):
            ...
            for n in range(0, len(columnsizes)):
                v = columnsizes[n]
                columnsizes[n] += old_div(v * d, w)
        if h > 0 and h < self.style.height and len(rowsizes):
            d = old_div((self.style.height - h), len(rowsizes))
            for n in range(0, len(rowsizes)):
                v = rowsizes[n]
                rowsizes[n] += old_div(v * d, h)

        # set the widget's position by calculating their row/column x/y offset
        cellpositions = [[[sum(columnsizes[0:cell]), sum(rowsizes[0:row])] for cell in range(self.getColumns())] for row
                         in range(self.getRows())]
        for row in range(self.getRows()):
            for cell in range(self.getColumns()):
                if self._rows[row][cell] and self._rows[row][cell] is not True:
...
class _Table_td(container.Container):
    ...
    def resize(self, width=None, height=None):
        ...
        dx = width - w.rect.w
        dy = height - w.rect.h
        w.rect.x = old_div((self.style.align + 1) * dx, 2)
        w.rect.y = old_div((self.style.valign + 1) * dy, 2)
