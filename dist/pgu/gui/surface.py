...
class ProxySurface(object):
    ...
    def subsurface(self, rect):
        return ProxySurface(self, pygame.Rect(rect).move(self.offset[0] + self.x, self.offset[1] + self.y),
                            self.real_surface)

    def fill(self, color, rect=None):
        if rect != None:
            self.mysubsurface.fill(color, rect)
        else:
            self.mysubsurface.fill(color)
    ...
    def get_abs_parent():
        return self.mysubsurface.get_abs_parent()

    def set_clip(self, rect=None):
        if rect == None:
            self.mysubsurface.set_clip()
        else:
