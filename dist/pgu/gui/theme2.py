...
class Theme(object):
    ...
    def resize(self, w, m):
        def func(width=None, height=None):
            ...
            # rect.x += (1)*dx/2
            # rect.y += (1)*dy/2
            rect.x += old_div((w.style.align + 1) * dx, 2)
            rect.y += old_div((w.style.valign + 1) * dy, 2)
    ...
    def open(self, w, m):
        def func(widget=None, x=None, y=None):
            if not hasattr(w,
                           '_rect_content'): w.rect.w, w.rect.h = w.resize()  # HACK: so that container.open won't resize again!
            rect = w._rect_content
    ...
    def render(self, s, box, r):
        ...
        ww, hh = old_div(box.get_width(), 3), old_div(box.get_height(), 3)
        xx, yy = x + w, y + h
