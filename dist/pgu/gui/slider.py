...
class _slider(widget.Widget):
    ...
    def paint(self, s):
        ...
        if self.orient == _SLIDER_HORIZONTAL:
            r.x = old_div((self.value - self.min) * (self.rect.w - self.size), (self.max - self.min));
            r.w = self.size;
        else:
            r.y = old_div((self.value - self.min) * (self.rect.h - self.size), (self.max - self.min));
            r.h = self.size;
            ...
            if 1 in e.buttons and self.container.myfocus is self:
                if self.grab != None:
                    ...
                    if self.orient == _SLIDER_HORIZONTAL:
                        ...
                        if d != 0: self.value = self.grab_value + (old_div((self.max - self.min) * rel[0], d))
                    else:
                        d = (self.rect.h - self.size)
                        if d != 0: self.value = self.grab_value + (old_div((self.max - self.min) * rel[1], d))
                else:
                    x, y, adj = e.pos[0], e.pos[1], 1

        if adj:
            if self.orient == _SLIDER_HORIZONTAL:
                d = old_div(self.size, 2) - old_div((old_div(self.rect.w, (self.max - self.min + 1))), 2)
                self.value = old_div((x - d) * (self.max - self.min), (self.rect.w - self.size + 1)) + self.min
            else:
                d = old_div(self.size, 2) - old_div((old_div(self.rect.h, (self.max - self.min + 1))), 2)
                self.value = old_div((y - d) * (self.max - self.min), (self.rect.h - self.size + 1)) + self.min
...
class HScrollBar(table.Table):
    ...
    def __setattr__(self, k, v):
        if k in ('min', 'max', 'value', 'size', 'step'):
            if k == 'size':
                v = old_div(v * self.slider.style.width, self.style.width)
...
class VScrollBar(table.Table):
    ...
    def __setattr__(self, k, v):
        if k in ('min', 'max', 'value', 'size', 'step'):
            if k == 'size':
                v = old_div(v * self.slider.style.height, self.style.height)
            return setattr(self.slider, k, v)
        self.__dict__[k] = v
