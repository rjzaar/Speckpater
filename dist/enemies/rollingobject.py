...
class RollingObject(BaseEnemy):
    def __init__(self, g, sprname, pos, initialXVel, hasFriction):
        def onCollision(g, s, a):
            velMul = old_div(int(math.sqrt(self.dx * self.dx + self.dy * self.dy)), 3)
    ...
    def __move(self, g):
        ...
        if self.rect.bottom < 0 or self.rect.right < 0 or self.rect.top >= (
                len(g.tlayer) * TILE_SIZE) or self.rect.left >= (len(g.tlayer[0]) * TILE_SIZE):
            # print "out of map"
    ...
    def __animate(self, g):
        ...
        rect = img.get_rect()
        ydif = old_div((rect.height - self.orgRect.height), 2)
        xdif = old_div((rect.width - self.orgRect.width), 2)
