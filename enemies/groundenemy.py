...
def getSpriteTileCoord(s):
    ...
    return Rect((old_div(s.rect.x, TILE_SIZE), old_div(s.rect.y, TILE_SIZE)),
                (int(old_div((s.rect.width - 1), TILE_SIZE)) + 1, int(old_div((s.rect.height - 1), TILE_SIZE)) + 1))
...
class GroundEnemy(BaseEnemy):
    ...
    def __init__(self, g, ishape, pos, wd, maxSpeed, maxAccel):
        ...
        self.wanderSpeed = old_div(self.maxSpeed, 2)
        self.xWanderAccel = old_div(self.xChaseAccel, 3)

        self.stateTimer = 0

        self.cdbgmsg = ""

        self.numTilesTall = old_div(self.rect.height, TILE_SIZE)
    ...
    def canMoveToDir(self, game, dir):
        try:
            feetY = old_div((self.rect.y + self.rect.height), TILE_SIZE)
            headY = old_div((self.rect.y), TILE_SIZE)

            tileX = 0
            tileColKeyName = ''

            if dir == -1:
                tileX = old_div((self.rect.x - 1), TILE_SIZE)
                ...
                tileX = old_div((self.rect.x + self.rect.width), TILE_SIZE)
